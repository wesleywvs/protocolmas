...
const SalesPage: React.FC = () => {
  ...
  const handleCheckout = () => {
    ...
    const checkoutUrl = `https://pay.cakto.com.br/39nsz8g_421258?external_id=${eventId}`;

    // 1. Rastreamento no Navegador (Pixel)
    if (typeof window.fbq === 'function') {
      window.fbq('track', 'InitiateCheckout', {
        content_name: 'Protocolo PDN',
        value: 27.00,
        currency: 'BRL'
      }, { eventID: eventId });
    }

    // 2. Rastreamento no Servidor (CAPI)
    const trackCapi = async () => {
      ...
      try {
        ...
        console.error('Falha ao enviar InitiateCheckout para CAPI:', error);
      }
    };
